  ...
  plugins: [
    {
      ...
      transformIndexHtml: {
        ...
        handler(html) {
          return html
            .replace(
              '<link rel="stylesheet" href="./assets/app-v8.css" />',
              ''
            )
            .replace(
              '<script type="module" src="./assets/app-v8.js"></script>',
    ...
    rollupOptions: {
      output: {
        ...
        assetFileNames: (assetInfo) => {
          if (assetInfo.name && assetInfo.name.endsWith('.css')) return 'assets/app-v8.css';
